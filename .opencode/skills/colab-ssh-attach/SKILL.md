...
## Defaults
- 短期主経路は Colab 出力の hostname を `--host` で渡す
- 接続情報の正本は `WORK_DIR/.colab_local/runtime/ssh_connection.json`（補助/診断用）
...
## Procedure
1. （短期既定）Colab 側で SSH セルを実行し、表示された `trycloudflare` の hostname を取得する。
2. ローカルで `scripts/colab_open_ssh_wezterm.sh --dry-run --host <hostname>` を実行して接続コマンドを確認する。
3. 問題なければ `scripts/colab_open_ssh_wezterm.sh --host <hostname>` を実行する。
4. `wezterm` 未導入時は `--print-only` または `--exec-shell` を使う。
5. 接続失敗時は URL の有効期限切れを疑い、Colab 側セルを再実行して hostname を更新する。
6. `ssh_connection.json` は診断や将来の自動化拡張時のみ使う。
